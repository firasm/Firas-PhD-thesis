...
The two animals presented here were selected to exhbit the full range of variability in extracted components.
Figure~\ref{numComponents} shows the the best- and worst-case scenarios for the extracted independent component as it may have implications when performing fits.
In both cases, the corresponding \acs{dOE-MRI} maps show the same O$_2$-positive and O$_2$-negative regions.
An important future work is to explore how the presence or absence of noise affects the extraction process.
