...
This value has been used to normalize relative tissue concentrations (Fig.~\ref{hpgpaper1:fig2}(B)) and to calculate the fractional plasma volume (\acs{fPV}; see Section 2.4).\todo{add proper ref here}
...
\begin{figure}[htbp]
 ...
 The concentration curves are shown as ratio of tissue concentration to blood peak concentration (24~$\mu$M) (C) The fraction of MR-measured \acs{HPG-GdF} \acs{BAT}-enhancing voxels has a negative association with the proportion of necrotic tissue determined in histological sections.}
...
\begin{figure}[htbp]
 ...
 \caption{Individual HT29 tumour (T01-05) slice maps are presented for parameters derived using Gadovist (\acs{BAT}, \acs{\acs{AUGC$_{60}$}$_{60}$}, \acs{K$^{trans}$}), \acs{HPG-GdF} (\acs{BAT}, \acs{fPV}, \acs{aPS}, \acs{\acs{AUGC$_{60}$}$_{60}$}) and histological imaging of necrosis.}
...
Administration of neither Gadovist nor \acs{HPG-GdF} was useful in detecting the difference in vascular function between HCT116 and HT29 tumours using the initial area under the gadolinium concentration curve (\acs{AUGC$_{60}$}), as the slice-averaged means were not significantly different between tumour groups (\acs{AUGC$_{60}$} for Gadovist in HCT116 was $5.09\pm 0.83$~mM$\cdot$s and \acs{AUGC$_{60}$} for Gadovist in HT29 was $7.68\pm 1.66$~mM$\cdot$s, ${p>0.05}$; \acs{AUGC$_{60}$} for \acs{HPG-GdF} in HCT116 was $6.41\pm 1.71\times 10^{-3}$~mM$\cdot$s and that in HT29 was $9.62\pm 1.64\times 10^{-3}$~mM$\cdot$s, ${p>0.05}$) (data summarized in Tables 2 and 3).
...
\begin{figure}[htbp]
 ...
 \caption{Vascular function in HCT116 xenografts. Whole-slice maps are presented for individual HCT116 tumours (T01-T05, vertical columns) for parameters derived from MR imaging of Gadovist (BAT, \acs{\acs{AUGC$_{60}$}$_{60}$}, K$^{trans}$), MR imaging of \acs{HPG-GdF} (\acs{BAT}, \acs{fPV}, \acs{aPS}, \acs{\acs{AUGC$_{60}$}$_{60}$}) and histological imaging (necrosis).}
