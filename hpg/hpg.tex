...
\todo[backgroundcolor=blue!20!white]{you previous sentence about appeal and this about it's applicability are saying the same thing. Explain more about WHY - more accurate diagnosis, better response times?? the next sentence sentence says in addition to it's utility as a biomarker. }
...
The issue of its limited access for anticancer drugs in solid tumours is significant~\cite{Minchinton:2006gs}, particularly given the efforts to create nanoparticle therapeutics that target the tumour via \added{the} \acs{EPR} effect~\cite{Jain:2001uf,Iyer:2006gf,Chauhan:2011fi}.
...
\todo[backgroundcolor=blue!20!white]{If you're going to define the smaller size, you should define the large size i.e. what is a dextran size}
...
\todo[backgroundcolor=blue!20!white]{ Somewhat sudden jump of measurements from kDa to nm. Shouldn't be something more consistent no?}
...
Signal enhancement is linked to contrast agent concentration within the tumour blood vessels at early time points after injection for intravascular \acs{MCA}s so plasma volumes (\acs{v$_p$})\SARi{why the exclamation mark?} in the tumour can be determined.
...
\SARi{various changes in the text below moslty messing with the inline equations. For instance use of tilde as a non-breaking space etc.}
...
During the first scanning session, T$_1$ and flip angle maps were acquired prior to \replaced{the}{a} DCE-MRI experiment, followed by another T$_1$ measurement.
At each imaging session slice location and orientation were adjusted to match previous sessions.
T$_1$ measurements and flip angle mapping were performed using a multi-slice FLASH variable flip angle experiment (FLASH TR/TE = 500/2.75, FA = 10$^{\circ}$, 20$^{\circ}$, 30$^{\circ}$, 40$^{\circ}$, 50$^{\circ}$, 60$^{\circ}$, 70$^{\circ}$, 80$^{\circ}$, 90$^{\circ}$, 100$^{\circ}$, 110$^{\circ}$, 120$^{\circ}$, 130$^{\circ}$, 140$^{\circ}$, 150$^{\circ}$, 160$^{\circ}$, 170$^{\circ}$, 180$^{\circ}$, 190$^{\circ}$, 200$^{\circ}$, 215$^{\circ}$) and data were fit simultaneously for T$_1$ and the \replaced{B$_1$}{B1} scaling factor map.
...
\todo[backgroundcolor=red!20!white]{consider updating this to the fixed version ?}
\begin{enumerate}
...
\end{enumerate}
\SARi{what do you mean fixed version??}

In the illustrated example (Fig.~\ref{hpgpaper1:fig2}(A)), a change from the mean was identified at Frame 31 and the two following scans by Rule (1); therefore, the \acs{BAT} point was selected as Frame 30. \SAR{indicate the time t=x s to which the frame corresponds since those are the units of your abscissa.}
...
\todo[backgroundcolor=red!20!white]{fix EA ref to aPS in the figure}
\begin{figure}[htbp]
 \begin{center}
...
\todo[backgroundcolor=red!20!white]{split these tables into 3 and reference correctly}
\SAR{what a hack! get those tables sorted!}
...
While the signal-to-noise \added{ratio} from \acs{HPG-GdF} concentration-time curves is not adequate to fit a complex pharmacokinetic model to determine parameters such as v$_p$, v$_e$ and K$^{trans}$, enhancement was measurable in most tumour voxels.
...
Limited extravasation of \acs{HPG-GdF} is likely due to the \highlight[comment={do you mean selective? or is it heterogenously permeable, or what}]{selective permeability} of tumour vessels with adequate pore sizes for passage of the bulky molecule, a phenomenon described as the enhanced permeability and retention (\acs{EPR}) effect~\cite{Maeda:2013hq}.
\SARi{I'd say, go back to Maeda and see what the EPR effect really is and what it says. In fact, is that where it's first defined?}
Limited distribution through the interstitium is also most probably the result of its significant size.
We have observed \acs{HPG-GdF} to be much more restricted in its distribution than is suggested by the \highlight[comment={how do you know D - I never knew we knew}]{diffusion coefficients} (D(HPG) = 3.7 \replaced{$\times$}{x} 10$^{-1}$7 cm$^2$ s$^{-1}$1 and D(Gadovist) = 3.6 \replaced{$\times$}{x} 10$^{-1}$6 cm$^2$ s$^{-1}$1), as the larger agent reaches only 10-15 $\mu$m away from vessels within an hour whereas Gadovist reaches all areas of a tumour.
\SARi{I think the exponents of the diffusion coefficients are off. e.g. 10 t.t.p.o. -11 and not 10 t.t.p.o. -1 space 1, also what about your exponents for seconds?}
...
\SARi{the whole chapter reads reasonably well minus the typesetting issues. I guess this has been read and edited by many before ...}
