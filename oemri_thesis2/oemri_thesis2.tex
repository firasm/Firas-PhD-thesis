...
\subsection{Theory: Modeling oxygen response}
...
This complex vascular tree is modeled by assuming a fractal branching geometry with the distribution of vessel sizes and flow rates given by a lognormal velocity distribution~\cite{Qian:2000ca}:
...
\begin{figure}[htbp]
   ...
   \caption{From Hudson et al 2009~\cite{Hudson:2009jv}; need to adapt and create or ask for permission to re-use}
   \label{lognormal}
\end{figure}

Figure~\ref{lognormal} shows a schematic of the vascular network modeled as a representative single vessel with a lognormal distribution of velocity profiles.
...
The SKOV3 tumours were not considered as part of this analysis due to a low sample size (N=3). 
