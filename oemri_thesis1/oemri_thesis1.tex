...
\textbf{dOE-MRI maps:} A suite of in-house software was developed using the python machine learning library scikit-learn~\cite{Pedregosa:2011tv}, specifically \texttt{sklearn.decomposition.Fast \ac{ICA}}\todo[backgroundcolor=green]{Have to figure out how to line break properly here} based on the technique described by Hyvarinen~\cite{Hyvarinen:2000vk}.
...
\subsection{ \ac{ICA} isolates small changes in T$_1$W signal intensity}
...
To retain spatial information and improve the sensitivity of the technique, \ac{ICA} was applied to the same dataset and in this example, four independent components were extracted (Figure~\ref{technique}C). 
...
\subsection{dOE-MRI with \ac{ICA} does not require assumption of a response function}
...
\subsection{Extracting the cycling component via \ac{ICA} is independent of number of components}
...
Two examples are shown, with a low and high degree of variability measured in the extracted components~\todo{SAR: very unsatisfying not the be told why there is a high and low variability version here. How come, what's the difference, what does it mean?}.
...
A full \ac{dOE-MRI} sequence involved three cycles of oxygen but to assess the potential for shortening the sequence we also separately applied \ac{ICA} to each of the three oxygen cycles independently.
...
In other words, temporal resolution could be sacrificed for SNR simply by averaging.
...
Details of this are discussed in section~\ref{sec:futureWork}~\todo[backgroundcolor=green]{Link this to future work about T2*}. 
...
Performing this analysis again with three-fold (Figure~\ref{fig_repeatability}) and six-fold (Appendix~\ref{sec:interleave}) temporally under-sampled data suggests that there is sufficient SNR to successfully extract the oxygen responsive component  with even a subset of the data.
