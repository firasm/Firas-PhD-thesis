...
Approximately X \todo[backgroundcolor=green]{how many days?} days later, tumours were imaged when their largest diameters reached approximately 8-10 mm.
...
To compare dOE-MRI maps between mice with different temporal resolutions, a scaling factor was applied (see section~\ref{sec:correctionfactor}).

\subsection{Fitting to lognormal perfusion model}

Blood flow in a vascular network has been modelled with a fractal branching geometry of a vascular tree and the distribution of vessel sizes and corresponding flow rates is given by a lognormal velocity distribution~\cite{Qian:2000ca}.



%%%% Equation 4 from Hudson paper, not really that relevant.
%\begin{equation}
%P(v) = \frac{1}{\sigma_f\sqrt{2\pi}} \cdot \frac{1}{v} exp\left[\frac{-\left(\frac{ln(v)-\mu_f}{\sigma_f}\right)^2}{2}   \right]
%\end{equation}
%
%where $P(v)$ is the probability density function of the velocity ($v$), $\mu_f$ and $\sigma_f$ are the distribution mean and standard deviation of the normal random variable $ln(v)$. 
...
\begin{figure}[htbp]
   \includegraphics[width=0.6\textwidth]{oemri_thesis1/oemri_thesis1-images/fig5_histocorrelation.pdf} % requires the graphicx package
